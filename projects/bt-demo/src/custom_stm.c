...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomStepHdle;                    /**< STEP_Service handle */
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for STEP_Service +
  ...
  uuid.Char_UUID_16 = 0x183e;
  aci_gatt_add_service(UUID_TYPE_16,
                       (Service_UUID_t *) &uuid,
                       PRIMARY_SERVICE,
                       10,
                       &(CustomContext.CustomStepHdle));

  /**
   *  X_acceleration
   */
  uuid.Char_UUID_16 = 0x1234;
  aci_gatt_add_char(CustomContext.CustomStepHdle,
                    UUID_TYPE_16, &uuid,
                    SizeX_Accel,
                    CHAR_PROP_NOTIFY,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch(CharOpcode)
  {
    ...
    case CUSTOM_STM_X_ACCEL:
      result = aci_gatt_update_char_value(CustomContext.CustomStepHdle,
                                          CustomContext.CustomX_AccelHdle,
                                          0, /* charValOffset */
